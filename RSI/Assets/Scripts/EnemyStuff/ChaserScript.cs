...
public class ChaserScript : BasicEnemy {
		...
		if(player!=null){
			...
			//If out of weapon range
				...
				if(isPlayerInRange(player,rangeOfVision,rangeOfVision)){
					...
					//Move x first, if not equal
					if(Mathf.Abs(transform.position.x-player.transform.position.x)>0.1f){
						
						Vector3 positionToGoTo = transform.position;
						
						
						positionToGoTo.x = player.transform.position.x;
						
						int val = Random.Range(0,100);
						
						float speed=mvmtSpeed;
						
						if(val>95){
							if(positionToGoTo.z<12){
								speed+=Random.value;
								positionToGoTo.z+= Random.value;
							}
						}
						else if(val<5){
							if(positionToGoTo.z>0){
								speed-=Random.value;
								positionToGoTo.z-= Random.value;
							}
						}
						transform.position = Vector3.Lerp(transform.position,positionToGoTo,speed*Time.deltaTime);
					}
					//If equal to z, move closer
					else{
						if((Mathf.Abs(player.transform.position.z-transform.position.z))>rangeOfWeapon){
